...
class VideoParser {
  ...
  static VideoDetailed? parsePlaylistVideo(dynamic item) {
    final flexColumns = (
            traverseList(item, ['flexColumns', 'runs']) is List
                ? traverseList(item, ['flexColumns', 'runs'])
                : (traverseList(item, ['flexColumns', 'runs']) is Map ? (traverseList(item, ['flexColumns', 'runs']) as Map).values : [traverseList(item, ['flexColumns', 'runs'])]))
        .expand((e) => e is List ? e : [e])
        .toList();
    final fixedColumns = (
            traverseList(item, ['fixedColumns', 'runs']) is List
                ? traverseList(item, ['fixedColumns', 'runs'])
                : (traverseList(item, ['fixedColumns', 'runs']) is Map ? (traverseList(item, ['fixedColumns', 'runs']) as Map).values : [traverseList(item, ['fixedColumns', 'runs'])]))
        .expand((e) => e is List ? e : [e])
        .toList();
